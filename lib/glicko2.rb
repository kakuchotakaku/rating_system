...
class Glicko2
  ...
  def p_win(opponent)
    1.0 / (1.0 + 10.0**(((opponent.rating - rating) / (400.0 * Math.sqrt(1.0 + 0.0000100723986 * (deviation**2.0 + opponent.deviation**2.0))))))
  end
  ...
  # util func 'g'
  def self.g(deviation)
    1.0 / Math.sqrt(1.0 + 3.0 * deviation**2.0 / (Math::PI**2.0))
  end

  # util func 'E'
  def self.E(rating, rating_opponent, deviation_opponent)
